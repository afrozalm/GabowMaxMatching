...
Below is a problem statement I came across while completing the foobar challenge and needed to find cardinality of maximum matching of the an undirected graph. I came across the paper by Harold N. Gabow which improves the original Edmonds' blossom based algorithm which reduces its complexity from `O(V^4)` to `O(V^3)`. Try out the problem for yourself before looking at the solution in example.py.
